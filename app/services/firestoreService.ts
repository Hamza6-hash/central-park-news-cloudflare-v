...
export const fireServices = {
  getDocuments: async (
    ...
    try {
      ...
      console.error(`Error fetching documents from ${collectionPath.join('/')}:`, error);
      throw error;
    }
  },

  getArticles: async (limitCount: number = 10): Promise<ArticleWithDetails[]> => {
    try {
      ...
      const articlesWithDetails = await Promise.all(articles.map(async (article) => {
        ...
        if (article.categoryId) {
          try {
            const categoryDoc = await getDoc(doc(db, 'blog', DOCKET_DIGEST, CATEGORIES, article.categoryId));
            if (categoryDoc.exists()) {
              category = { id: categoryDoc.id, ...categoryDoc.data() } as Category;
            }
          } catch (categoryError) {
            console.error('Error fetching category for article:', article, categoryError);
          }
        }

        if (article.authorId) {
          try {
            const authorDoc = await getDoc(doc(db, 'blog', DOCKET_DIGEST, AUTHORS, article.authorId));
            if (authorDoc.exists()) {
              author = { id: authorDoc.id, ...authorDoc.data() } as Author;
            }
          } catch (authorError) {
            console.error('Error fetching author for article:', article, authorError);
          }
        }
  ...
  getAuthor: async (limitCount: number = 5): Promise<Author[]> => {
    return fireServices.getDocuments(['blog', DOCKET_DIGEST, AUTHORS], AUTHORS_ORDER_BY_FIELD, limitCount) as Promise<Author[]>;
  },

  getCategories: async (limitCount: number = 5): Promise<Author[]> => {
    return fireServices.getDocuments(['blog', DOCKET_DIGEST, CATEGORIES], CATEGORIES_ORDER_BY_FIELD, limitCount) as Promise<Author[]>;
  },

  searchArticles: async (searchTerm: string, limitCount: number = 10): Promise<ArticleWithDetails[]> => {
    try {
      const articlesRef = collection(db, 'blog', DOCKET_DIGEST, ARTICLES);
  ...
  getFeaturedArticles: async (): Promise<ArticleWithDetails[]> => {
    ...
    const articlesWithDetails = await Promise.all(featuredArticles.map(async (article) => {
      let category = null;
      let author = null;
