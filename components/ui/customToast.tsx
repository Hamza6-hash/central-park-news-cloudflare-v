import { SquareCheck, AlertCircle, X } from 'lucide-react';
...
interface ToastProps {
  ...
  type?: 'success' | 'error' | 'alreadyUnsubscribed';
}
...
const CustomToast: React.FC<ToastProps> = ({ 
  ...
  const isError = type === 'error' || type === 'alreadyUnsubscribed';
  ...
  const icon = isError ? <AlertCircle size={20} strokeWidth={1.5} /> : <SquareCheck size={20} strokeWidth={1.5} />;
  
  return (
    <div className="fixed top-4 right-4 md:bottom-6 md:top-auto md:right-6 z-50 w-full max-w-[350px] md:max-w-[400px] px-4 md:px-0">
